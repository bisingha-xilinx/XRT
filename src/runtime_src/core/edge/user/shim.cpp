...
int shim::xclPrepareAxlf(const axlf *buffer, struct drm_zocl_axlf *axlf_obj)
{
  int ret = 0;
  unsigned int flags = DRM_ZOCL_PLATFORM_BASE;
  int off = 0;
...
int shim::xclLoadHwAxlf(xclDeviceHandle handle, const xclBin *buffer, drm_zocl_create_hw_ctx *hw_ctx)
{
  xclLog(XRT_ERROR, "++ %s: %d buffer:%s", __func__, __LINE__, buffer);
  drm_zocl_axlf axlf_obj = {};
  auto top = reinterpret_cast<const axlf*>(buffer);
  auto ret = xclPrepareAxlf(top, &axlf_obj);
  if (ret)
    return -errno;
                  ...
                  xrt::hw_context::access_mode mode)
{
  ...
  xclLog(XRT_ERROR, "++ %s, buffer: %s", __func__, buffer);
  if(auto ret = shim->xclLoadHwAxlf(handle, buffer, &hw_ctx)) {
    if (ret) {
      if (ret == -EOPNOTSUPP) {
  ...
  return std::make_unique<hwcontext>(this, hw_ctx.hw_context, xclbin_uuid, mode);
}
